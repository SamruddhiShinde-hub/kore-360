  ...
  return (
    ...
    >
      <Reveal>
        ...
        <p style={{ fontSize: 'clamp(16px,1.5vw,20px)', lineHeight: 1.55, color: '#A0A0A0', maxWidth: '480px', margin: '0 0 34px' }}>{HERO.blurb}</p>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '14px' }}>
          <a href={LINKS.bookCall} target="_blank" rel="noreferrer" className="btn-accent" style={btnAccent}>{HERO.primaryCta}</a>
          <a href={LINKS.ebook} target="_blank" rel="noreferrer" className="btn-outline" style={{ display: 'inline-flex', alignItems: 'center', fontSize: '16px', fontWeight: 700, color: '#FFFFFF', background: 'transparent', border: '1px solid rgba(255,255,255,0.22)', padding: '15px 26px', borderRadius: '8px' }}>{HERO.secondaryCta}</a>
        </div>
      </Reveal>
      <Reveal delay={1} style={{ position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', top: '18px', right: '-14px', background: '#000000', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', padding: '12px 16px', boxShadow: '0 18px 40px rgba(0,0,0,0.4)' }}>
          <div style={{ fontWeight: 900, fontSize: '26px', color: 'var(--kore-pink)' }}>100+</div>
          <div style={{ fontSize: '11px', color: '#A0A0A0', letterSpacing: '0.04em' }}>placed in sports</div>
        </div>
        <div style={{ position: 'absolute', bottom: '24px', left: '-16px', background: '#000000', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', padding: '12px 16px', boxShadow: '0 18px 40px rgba(0,0,0,0.4)' }}>
          <div style={{ fontWeight: 900, fontSize: '26px', color: '#FFFFFF' }}>60+</div>
          <div style={{ fontSize: '11px', color: '#A0A0A0', letterSpacing: '0.04em' }}>1:1 calls taken</div>
        </div>
      </Reveal>
    </div>
  );
...
function HeroCentered() {
  ...
  return (
    <div style={{ position: 'relative', minHeight: '86vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '96px 32px 72px', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg,rgba(0,0,0,0.72) 0%,rgba(0,0,0,0.64) 42%,rgba(0,0,0,0.94) 100%)' }} />
      <Reveal style={{ position: 'relative', zIndex: 2, textAlign: 'center', maxWidth: '1040px' }}>
        ...
        <h1 style={{ fontWeight: 900, textTransform: 'uppercase', letterSpacing: '-0.02em', lineHeight: 0.88, fontSize: 'clamp(52px,10vw,150px)', margin: '0 0 26px', textShadow: '0 4px 40px rgba(0,0,0,0.5)' }}>
          <HeadingBreaks lines={HERO.headlineLines} />
        </h1>
        <p style={{ fontSize: 'clamp(16px,1.5vw,21px)', lineHeight: 1.55, color: '#FFFFFF', maxWidth: '620px', margin: '0 auto 34px' }}>{HERO.blurb}</p>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '14px', justifyContent: 'center' }}>
          <a href={LINKS.bookCall} target="_blank" rel="noreferrer" className="btn-accent" style={btnAccent}>{HERO.primaryCta}</a>
          <a href={LINKS.ebook} target="_blank" rel="noreferrer" className="btn-outline" style={{ display: 'inline-flex', alignItems: 'center', fontSize: '16px', fontWeight: 700, color: '#FFFFFF', border: '1px solid rgba(255,255,255,0.35)', background: 'rgba(0,0,0,0.3)', padding: '15px 26px', borderRadius: '8px' }}>{HERO.secondaryCta}</a>
        </div>
      </Reveal>
      <Reveal delay={1} style={{ position: 'relative', zIndex: 2, display: 'flex', justifyContent: 'center', gap: '14px', flexWrap: 'wrap', marginTop: '52px' }}>
        {HERO.stats.map((s, i) => (
          <div key={i} style={{ background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: '10px', padding: '14px 24px', textAlign: 'center' }}>
            <div style={{ fontWeight: 900, fontSize: '28px', color: i === 0 ? 'var(--kore-purple)' : i === 2 ? 'var(--kore-pink)' : '#FFFFFF' }}>{s.value}</div>
            <div style={{ fontSize: '11px', color: '#A0A0A0' }}>{s.label}</div>
          </div>
        ))}
      </Reveal>
    </div>
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '64px 32px 40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', borderBottom: '1px solid rgba(255,255,255,0.12)', paddingBottom: '18px', marginBottom: '40px' }}>
        ...
        <div style={{ fontSize: '13px', letterSpacing: '0.16em', color: '#A0A0A0' }}>SPORTS MANAGEMENT</div>
      </div>
      <div className="grid-2col" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '48px', alignItems: 'end' }}>
        <Reveal>
          ...
          <p style={{ fontSize: 'clamp(16px,1.4vw,20px)', lineHeight: 1.55, color: '#A0A0A0', maxWidth: '520px', margin: '0 0 30px' }}>{HERO.blurb}</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '14px' }}>
            <a href={LINKS.bookCall} target="_blank" rel="noreferrer" className="btn-accent" style={btnAccent}>{HERO.primaryCta}</a>
            <a href={LINKS.ebook} target="_blank" rel="noreferrer" className="btn-outline" style={{ display: 'inline-flex', alignItems: 'center', fontSize: '16px', fontWeight: 700, color: '#FFFFFF', border: '1px solid rgba(255,255,255,0.22)', padding: '15px 26px', borderRadius: '8px' }}>{HERO.secondaryCta}</a>
          </div>
        </Reveal>
