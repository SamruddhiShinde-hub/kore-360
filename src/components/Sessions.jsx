...
const eyebrow = { fontSize: '13px', letterSpacing: '0.16em', color: 'var(--kore-orange)', marginBottom: '16px' };
const CARD_COLORS = ['var(--kore-orange)', 'var(--kore-magenta)', 'var(--kore-orange)', 'var(--kore-magenta)'];
    ...
    <section id="sessions" style={{ borderBottom: '1px solid rgba(var(--border-rgb),0.08)' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '92px 32px' }}>
        ...
        <Reveal style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(248px,1fr))', gap: '18px' }}>
          {SESSIONS.map((s, i) => (
            <div key={i} className="session-card" style={{ position: 'relative', display: 'flex', flexDirection: 'column', background: 'var(--surface)', border: '1px solid rgba(var(--border-rgb),0.09)', borderRadius: '14px', padding: '26px', minHeight: '340px' }}>
              {s.featured && (
              ...
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '16px' }}>{s.meta}</div>
