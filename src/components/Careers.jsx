...
const CARD_COLORS = ['var(--kore-orange)', 'var(--kore-magenta)'];
    ...
    <section id="careers" style={{ borderBottom: '1px solid rgba(var(--border-rgb),0.08)' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '92px 32px' }}>
        ...
        <Reveal style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: '20px', marginBottom: '20px' }}>
          {CAREERS.map((k, i) => (
            <div key={i} className="card-hover" style={{ display: 'flex', flexDirection: 'column', background: 'var(--surface)', border: '1px solid rgba(var(--border-rgb),0.09)', borderRadius: '14px', padding: '30px' }}>
        ...
        <Reveal className="grid-2col" style={{ background: 'var(--surface)', border: '1px solid rgba(var(--border-rgb),0.09)', borderRadius: '14px', padding: '34px', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '36px', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ fontSize: '15px', lineHeight: 1.55, color: 'var(--text-muted)', margin: 0 }}>Drop your resume and we'll reach out when the right internship or job opens up. No spam — just real opportunities in sport.</p>
          </div>
